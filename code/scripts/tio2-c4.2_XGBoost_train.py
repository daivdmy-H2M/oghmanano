...
OUTPUT_BASE_DIR = PROJECT_ROOT / "bin" / "tio2_c_4.2"
...
def iqr_mask(series: pd.Series, k: float = 3.0) -> pd.Series:
    ...
    lower = q1 - k * iqr
    upper = q3 + k * iqr
    return clean.between(lower, upper)


def filter_plot_pairs_by_iqr(true_values, pred_values, k: float = 3.0):
    pair_df = pd.DataFrame({"true": true_values, "pred": pred_values}).dropna()
    if pair_df.empty:
        return pair_df

    true_mask = iqr_mask(pair_df["true"], k=k)
    pred_mask = iqr_mask(pair_df["pred"], k=k)
    return pair_df.loc[true_mask & pred_mask].reset_index(drop=True)


def plot_compare(train_true, train_pred, test_true, test_pred, title, output_path: Path):
    train_df = filter_plot_pairs_by_iqr(train_true, train_pred, k=3.0)
    test_df = filter_plot_pairs_by_iqr(test_true, test_pred, k=3.0)
